...
class GeneradorHTML:
    def __init__(self, nombre_de_archivo, datos):
        self.nombre_archivo = nombre_de_archivo + '/' + nombre_de_archivo + '.html'
        self.name_default = nombre_de_archivo + '/' + 'index.html'
        self.datos = datos

    def generar_archivo(self):
        archivo_html = open(self.name_default, 'w')
        with open('BaseHTML/base_html', 'r') as file_codigo:
    ...
    def pasar_datos_dic(self, datos_tupla):
        dic = {'url': datos_tupla[0], 'usuario': datos_tupla[1],
               'fecha': '{0}/{1}/{2}'.format(self.formato_numero(datos_tupla[4]),
                                             self.formato_numero(datos_tupla[5]),
                                             datos_tupla[6]),
               'descripcion': datos_tupla[2], 'tags': datos_tupla[3],
